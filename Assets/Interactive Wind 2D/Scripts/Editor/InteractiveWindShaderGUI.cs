    ...
    public class InteractiveWindShaderGUI : ShaderGUI
    {
        ...
        public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
        {
            ...
            for(int n = 0; n < properties.Length; n++)
            {
                ...
                {
                    propType = ShaderUtil.GetPropertyType(shader, n);
                }
                ...
                #region Reset
                if (propType != ShaderUtil.ShaderPropertyType.TexEnv && displayName != "Shader Space" && prop.name.StartsWith("_Enable") == false && prop.name != "_WindIsParallax" && prop.name != "_WindLocalWind" && prop.name != "_WindHighQualityNoise")
                {
